...
sgx_status_t ecall_add_gen_V(char *enc_st, int data_size, char *cuV, int uVlen, char *ceV, int eVlen){
    ...
    cnt++;
    I.insert(std::make_pair(ind, cnt));

    std::string sign = "*";
    std::string dot = ",";
    Istr.append(ind);
    Istr.append(dot);
    Istr.append(std::to_string(cnt));
    Istr.append(sign);
...
sgx_status_t ecall_update_D(char *enc_st, 
    ...
    std::string kw = sse_crypto_manager->aes_encrypt((unsigned char*)kT.c_str(), (unsigned char*)ivT.c_str(), w);//生成kw
    std::string suT;
    GenuT(kw, dind, suT);
    memset(uT, 0, uTlen);
...
sgx_status_t ecall_search_w(char *enc_st, int data_size, char *xuT, int len, char *xNum, int lenNum) {
    ...
    for (int i = 0; i < inw; i++){
        for (auto it : D) {
            h = it.find(',', 0);
            std::string sind = it.substr(0, h);
            std::string scnt = it.substr(h + 1);
            cond1 = w.compare(sind);
            cond2 = (Num[i] < atoi(scnt.c_str()));
            cond = cond1 || cond2;
            Num[i] += int(!cond);
        }
    }
